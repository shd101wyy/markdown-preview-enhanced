...
Markdown Preview Enhanced は [D2](https://d2lang.com/) を使用して図をレンダリングします。D2 はテキストを図に変換する宣言型ダイアグラム言語です。

- [D2](https://d2lang.com/) がインストールされ、`PATH` が通っている必要があります（または設定の `d2Path` で構成可能）。
- `d2` 表記のコードブロックは [D2](https://d2lang.com/) によってレンダリングされます。
- 完全な構文リファレンスについては、[D2 ドキュメント](https://d2lang.com/tour/intro/) を参照してください。

コードブロックごとにレイアウトエンジン、テーマ、スケッチスタイルを上書きできます：
![d2-test](https://github.com/user-attachments/assets/809cc0e7-e7a0-4637-9a4d-3992edab725d)

| 属性       |  description                                                      | デフォルト |
| --------- | ------------------------------------------------------------------ | ------- |
| `layout`  | レイアウトエンジン：`dagre`、`elk`、`tala`                              | `dagre` |
| `theme`   | テーマ ID 番号（[D2 テーマ](https://d2lang.com/tour/themes/) を参照）  | `0`     |
| `sketch`  | 手書き/スケッチスタイルでレンダリング                                    | `false` |

グローバルデフォルトは[パッケージ設定](ja-jp/usages.md?id=package-settings)で構成できます：

| 設定        | 説明                        | デフォルト |
| ---------- | --------------------------- | ------- |
| `d2Path`   | `d2` 実行ファイルへのパス      | `d2`    |
| `d2Layout` | デフォルトのレイアウトエンジン  | `dagre` |
| `d2Theme`  | デフォルトのテーマ ID          | `0`     |
| `d2Sketch` | デフォルトのスケッチモード      | `false` |

> **注：** D2 のレンダリングには、マシンに `d2` CLI がインストールされている必要があります。見つからない場合、コードブロックはプレーンテキストとして表示されます。インストール手順については、[D2 インストールガイド](https://d2lang.com/tour/install/) を参照してください。

## TikZ

Markdown Preview Enhanced は `tikz` フェンス付きコードブロックを介して [TikZ](https://tikz.dev/) 図のレンダリングをサポートします。

- Node.js（デスクトップ VS Code）: [node-tikzjax](https://github.com/prinsss/node-tikzjax) を使用してサーバーサイドで TikZ を SVG にレンダリングし、キャッシュします。
- Web（VS Code Web 拡張機能）および HTML エクスポート: [tikzjax.com](https://tikzjax.com) を介したクライアントサイドレンダリングにフォールバックします。
- コードに `\begin{document}...\end{document}` がない場合は自動的に挿入されます。
- ベース TeX パッケージを自動ロード: `amsmath`、`amssymb`、`amsfonts`、`amstext`、`array`。
- 特殊パッケージを自動検出してロード: `tikz-cd`（`\begin{tikzcd}` 用）、`pgfplots`（`\begin{axis}` 用）、`circuitikz`（`\begin{circuitikz}` 用）、`chemfig`（`\chemfig` 用）、`tikz-3dplot`（`\tdplotsetmaincoords` 用）。

フェンス情報文字列でサポートされるブロックごとのオプション：

| オプション | 説明 | 受け入れられる値 |
| ------ | ----------- | --------------- |
| `texPackages` / `tex_packages` | 追加でロードする TeX パッケージ | カンマ区切りリスト |
| `tikzLibraries` / `tikz_libraries` | ロードする TikZ ライブラリ | カンマ区切りリスト |
| `addToPreamble` / `add_to_preamble` | プリアンブルに追加するカスタム LaTeX コード | LaTeX 文字列 |
| `showConsole` / `show_console` | コンソール出力を表示 | `true` / `false` |
| `embedFontCss` / `embed_font_css` | フォント CSS を埋め込む | `true` / `false` |
| `fontCssUrl` / `font_css_url` | カスタムフォント CSS URL | URL 文字列 |

> **注：** TikZ レンダリングは、クライアントサイドレンダリング（tikzjax.com）のためにネットワークアクセスが必要です。node-tikzjax を使用したサーバーサイドレンダリングは、初期セットアップ後はオフラインでも動作します。

## WebSequenceDiagrams

Markdown Preview Enhanced は `wsd` フェンス付きコードブロックを介して [WebSequenceDiagrams](https://www.websequencediagrams.com/) のレンダリングをサポートします。

- `wsd` 表記のコードブロックは [WebSequenceDiagrams](https://www.websequencediagrams.com/) によってレンダリングされます。
- オプションの API キーは[パッケージ設定](ja-jp/usages.md?id=package-settings)で構成できます。

> **注：** WebSequenceDiagrams のレンダリングには、websequencediagrams.com へのネットワークアクセスが必要です。
