...
// [["$", "$"], ["\(", "\)"]]
atom.config.observe('markdown-preview-enhanced.indicatorForMathRenderingInline', (indicatorStr)=> {
  try {
    let indicators = JSON.parse(indicatorStr).filter(x => x.length === 2)
...
atom.config.observe('markdown-preview-enhanced.indicatorForMathRenderingBlock', (indicatorStr)=> {
  try {
    let indicators = JSON.parse(indicatorStr).filter(x => x.length === 2)
...
let md = new remarkable('full',defaults)
...
md.inline.ruler.before('escape', 'math', function(state, silent) {
  ...
  for (let i = 0; i < block.length; i++) {
    if (state.src.startsWith(block[i][0], state.pos)) {
      openTag = block[i][0]
      closeTag = block[i][1]
      displayMode = true
      break
  ...
  if (!openTag) {
    for (let i = 0; i < inline.length; i++) {
      if (state.src.startsWith(inline[i][0], state.pos)) {
        openTag = inline[i][0]
        closeTag = inline[i][1]
        displayMode = false
        break
  ...
  let content = null
  let end = -1
  for (let i = state.pos + openTag.length; i < state.src.length; i++) {
...
md.inline.ruler.before('autolink', 'wikilink', function(state, silent) {
  if (!enableWikiLinkSyntax) return false
...
md.block.ruler.before('code', 'custom-comment', function(state, start, end, silent) {
  ...
  if (state.src.startsWith('<!--', pos)) {
    ...
    if (end >= 0) {
      let contents = state.src.slice(pos + 4, end).trim().split(' ')

      let content = contents[0]
      let option = {}
      for (let i = 1; i < contents.length; i++) {
        let o = contents[i].split(':')
        if (o.length == 2) {
          option[o[0]] = o[1]
        } else {
          option[o[0]] = null
...
function resolveImagePathAndCodeBlock(html, markdownPreview, option) {
  option = option || {}
...
function parseMD(markdownPreview, option) {
  option = option || {isSavingToHtml: false}
  let editor = markdownPreview.editor

  let inputString = editor.getText()
  let headings = []
  ...
  md.renderer.rules.custom = (tokens, idx)=> {
    ...
    if (content === 'pagebreak' || content === 'newpage') {
      ...
      if (tocStartLine === -1) {
        ...
        let option = tokens[idx].option
        if (option.orderedList && option.orderedList !== '0') {
          tocOrdered = true
        }
      } else {
  ...
  if (tocEnabled) {
    ...
    if (tocNeedUpdate && editor) {
      let tocObject = toc(headings, tocOrdered)
