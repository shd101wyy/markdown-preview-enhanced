...
let katex = require('katex'), // for katex
    ...
    mathRenderOption = null

mermaidAPI.initialize({
  startOnLoad: false
})

atom.config.observe('atom-markdown-katex.mathRenderOption', (option)=> {
  if (option === 'None') {
    mathRenderOption = null
  } else {
    mathRenderOption = option

    if (mathRenderOption[0] === 'M') { // MathJax
...
md.inline.ruler.before('autolink', 'math', function(state, silent) {
  if (mathRenderOption !== 'KaTeX') return false
