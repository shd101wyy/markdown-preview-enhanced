...
md.inline.ruler.before('autolink', 'math', function(state, silent) {
  console.log(state, state.pos, state.src[state.pos])
  ...
  let end = state.src.indexOf(tag, state.pos + tag.length)
  if (end >= 0) {
...
// toc
md.core.ruler.before('block', 'toc', function(state, idx) {
  if (!state.tokens.length || !state.tokens[idx]) return false
  let content = state.tokens[idx].content
...
md.renderer.rules.paragraph_open = function (tokens, idx) {
  ...
  if (tokens[idx + 1] && tokens[idx + 1].content) {
    let content = tokens[idx + 1].content.trim()
    console.log(content)
    if (content === '\\pagebreak') {
      return '<p class="pagebreak">'
    }
  }
  */
...
function parseMD(markdownPreview) {
  ...
  md.renderer.rules.toc_enable = (tokens, idx)=> {
