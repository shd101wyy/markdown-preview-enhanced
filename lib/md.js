...
md.block.ruler.before('code', 'custom-comment', function(state, start, end, silent) {
  /*
  console.log(state, idx, state.tokens.slice(0), arguments)
  ...
  if (content.startsWith('<!--')) {
    ...
    if (end >= 0) {
      state.push({
        type: 'custom',
        content: content.slice(4, end).trim()
      })
      return true
    } else {
      return false
...
function parseMD(markdownPreview) {
  ...
  if (tocEnabled) {
    ...
    if (tocNeedUpdate) {
      ...
      let replaced = toc.insert(inputString, {slugify: uslug})

      tocTable = {} // reset tocTable before md
      tocEnabled = false
      html = md.render(replaced)
      markdownPreview.editor.setText(replaced)
