...
class MarkdownPreview {
  constructor(editor) {
    ...
    this.uri = 'atom-markdown-katex://' + editor.getFileName() + ' preview'
    ...
    atom.workspace.onDidChangeActivePaneItem((item)=> {
      if (item && item === this.previewEditor && this.htmlView && this.domContainer) {
        this.domContainer.style.display = 'inline-flex'
      }
    })
  ...
  createPreviewElement() {
    let textEditorElement = atom.views.getView(this.previewEditor)
    let parentElement = textEditorElement.parentElement

    this.htmlView = document.createElement('div')
    this.htmlView.style.width = '100%'
    this.htmlView.style.height = '96%' // cannot use 100%... use 100% doesn't display well
    ...
    this.domContainer.style.margin = '0'
    this.domContainer.style.padding = '0' // this one is important
    this.domContainer.style.zIndex = '999'
    this.domContainer.style.overflow = 'hidden'
    this.domContainer.style.display = 'inline-flex'
    this.domContainer.className = 'markdown-katex-preview'

    parentElement.insertBefore(this.domContainer, textEditorElement)
    parentElement.removeChild(textEditorElement)

    let shadowDom = this.domContainer.createShadowRoot()
