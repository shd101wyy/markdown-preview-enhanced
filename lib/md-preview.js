...
class MarkdownPreview {
  constructor(editor) {
    ...
    this.resizeEvent = ()=> { // when resizing window, clear this.sourceMap
      this.sourceMap = null
    }
  ...
  connectPreviewEditor(previewEditor) {
    ...
    setTimeout(()=> {
      ...
      if (this.editor && this.htmlView) {
        this.scrollSyncToLineNo(this.editor.getCursorScreenPosition().row)
      }
    }, 500)
  }
  ...
  createPreviewElement() {
    ...
    if (!globalKatexStyle) {
      ...
      let globalMermaidStyle = document.createElement('link')
      globalMermaidStyle.rel = 'stylesheet'
      globalMermaidStyle.href = path.resolve(__dirname,
      '../node_modules/mermaid/dist/mermaid.css')
      document.getElementsByTagName('head')[0].appendChild(globalMermaidStyle)
    }
    ...
    katexStyle.innerHTML = `@import url("${path.resolve(__dirname, '../node_modules/katex/dist/katex.min.css').replace(/\\/g, '/')}");`
  ...
  initEditorEvent() {
    ...
    this.disposables.add(
      this.editor.onDidDestroy(()=> {
        ...
        if (atom.config.get('atom-markdown-katex.closePreviewWhenClosingEditor') && this.previewEditor) {
  ...
  renderMermaid() {
    let els = this.htmlView.querySelectorAll('.mermaid')
    for (let i = 0; i < els.length; i++) {
      let e = els[i],
          id = 'mermaid-' + i + '-' + this.instanceId

      e.id = id
      mermaidAPI.render(id, e.textContent, function(html){e.innerHTML = html }, e)
    }
  }
