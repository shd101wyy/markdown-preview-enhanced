...
let md = require('./md.js'),
    ...
    processMath = require('./mathjax-wrapper').processMath,
    parseMD = md.parseMD,
...
class MarkdownPreview {
  ...
  initSettingsEvents() {
    ...
    // KaTeX?
    this.disposables.add(atom.config.observe('atom-markdown-katex.mathRenderOption', (option)=> {
      setTimeout(()=> {
        this.mathRenderOption = option
        this.renderMarkdown()
      }, 500)
    }))
  }
  ...
  scrollSyncToLineNo(lineNo) {
    ...
    let firstVisibleScreenRow = this.editor.getFirstVisibleScreenRow(),
        ...
        scrollTop = this.sourceMap[lineNo] - (posRatio > 1 ? 1 : posRatio) * editorElement.getHeight()
  ...
  renderMathJax() {
    if (this.mathRenderOption && this.mathRenderOption[0] === 'M') {// MathJax
      processMath(this.htmlView)
    }
  }
  ...
  renderMarkdown() {
      ...
      if (this.mathRenderOption === 'MathJax' && MathJax) {
