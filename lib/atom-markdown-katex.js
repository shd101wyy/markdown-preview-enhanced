...
function parseKatex(input_string, err_obj) {
  output_string = ""
  // return end
    ...
    if (input_string[i] === "\\") {
      ...
      if (end !== -1) {
        try {
          ...
          err_obj[0] = input_string.slice(start, end)
          return (void 0);
        }
        i = end + tag.length - 1
      } else {
...
function parseMD(input_string, directoryPath) {
  ...
  var err_obj = []
  var replaced = parseKatex(input_string, err_obj)
  if (replaced === void 0)
...
var activate_fn = function() {
  atom.commands.add("atom-workspace", "markdown-katex-preview:save-as-pdf", saveAsPDF)
