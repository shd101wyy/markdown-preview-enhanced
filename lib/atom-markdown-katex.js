...
// Stop using the "marked" above because the official one doesn't support "lineNumbers" option
// check https://github.com/chjj/marked/pull/680 this pull request
var marked = require("./marked.js") // for markdown
...
var $ = null
var sourceMap = null // sourceMap for scroll sync
        ...
        */
        ...
        activeEditor.onDidChangeCursorPosition(function(event) {
          if (activeEditor && activeEditor.markdownHtmlView) {
            var lineNumber = Math.floor(event.newBufferPosition.row) + 1 // cursor position

            for(var i = -10; i < 10; i++) {
              var span = document.querySelector('span[line-number="'+(i + lineNumber)+'"]')
              if (span) {
                console.log('fins span, ', span)
                activeEditor.markdownHtmlView.scrollTop = span.previousElementSibling.offsetTop
                return;
              }
            }
          }
        ...
        */
        ...
        activeEditor.onDidChangeScrollTop(function() {
          if (activeEditor && activeEditor.markdownHtmlView) {
            ...
            if (activeEditor.getScrollBottom() + 6 >= activeEditor.getScrollHeight()) {
              activeEditor.markdownHtmlView.scrollTop = activeEditor.markdownHtmlView.scrollHeight
              return
            }

            var firstVisibleScreenRow = activeEditor.getFirstVisibleScreenRow()
            var lastVisibleScreenRow = activeEditor.firstVisibleScreenRow + Math.floor(activeEditor.getHeight() / activeEditor.getLineHeightInPixels()) // lastVisibleScreenRow doesn't work

            var firstVisibleBufferRow = activeEditor.bufferRowForScreenRow(firstVisibleScreenRow)
            var lastVisibleBufferRow = activeEditor.bufferRowForScreenRow(lastVisibleScreenRow)

            var lineNumber = Math.floor((firstVisibleBufferRow + lastVisibleBufferRow) / 2)
            ...
            for(var i = 0; i < 10; i++) {
              var span = document.querySelector('span[line-number="'+(i + lineNumber)+'"]')
              if (span) {
                // console.log('find span, ', span)
                activeEditor.markdownHtmlView.scrollTop = span.previousElementSibling.offsetTop - activeEditor.markdownHtmlView.offsetHeight/2
                return;
              }
            }
            // console.log('didnt find, ', lineNumber)
          }
        })
