...
function buildScrollMap(editor) {
  ...
  /*
  console.log('here', _scrollMap, nonEmptyList)
  let els = markdownHtmlView.querySelectorAll('h1, h2, h3, h4, h5, h6, pre, .line, p')
  let lines2 = lines.filter(str => str.length > 0)
  let map = {}, c = 0
  let els2 = []
  for(let i = 0; i < els.length; i++) {
    if (els[i].textContent.length > 0 || els[i].getAttribute('data-line')) {
      els2.push(els[i])
    }
  }
  console.log('#########')
  ...
  lines2.forEach((str) => {
    ...
    if (str.trim().indexOf('```') === 0) {
      if (code === 0) {
        ...
        map[str] = els2[c].offsetTop + els2[c].offsetHeight
        code = 0
        c++
        return
      }
    ...
    .then(function(editor) {
      if (activeEditor.markdownHtmlView === null) {
        ...
        activeEditor.onDidChangeCursorPosition(function(event) {
          ...
          if (!event.textChanged && event.oldScreenPosition.row !== event.newScreenPosition.row) {
            if (!activeEditor.sourceMap) {
              activeEditor.sourceMap = buildScrollMap(activeEditor)
            }
            let lineNo = event.newScreenPosition.row,
                ...
                scrollTop = activeEditor.sourceMap[lineNo] - (lineNo - firstVisibleScreenRow) / (activeEditor.getHeight() / activeEditor.getLineHeightInPixels()) * activeEditor.getHeight()
